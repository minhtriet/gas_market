...
def _lstm_cell(layer_shape):
    # return [tf.contrib.rnn.LSTMBlockCell(num_unit) for num_unit in layer_shape]
    return [tf.nn.rnn_cell.LSTMCell(num_unit) for num_unit in layer_shape]
    ...
    with tf.Session() as sess:
        ...
        for epoch_step in range(epoch):
            ...
            if epoch_step % 5 == 0:
                ...
                [train_acc, s] = sess.run([train_accuracy, summ], feed_dict={inputs: x_train, targets: y_train})
                train_writer.add_summary(s, global_step=epoch_step)
                val_acc = sess.run(validation_accuracy, feed_dict={inputs: x_test, targets: y_test})
